...
エージェント間で「成果物」だけを渡すと、次のエージェントは前のエージェントが**なぜそう判断したか**を知ることができない。各エージェントの内部推論（Chain of Thought）はコンテキストウィンドウの中に閉じており、ファイルに書き出さない限り他のエージェントには伝わらないためである。

peer-to-peer 通信（Agent Teams など）でも同じ制約があり、エージェントが明示的に言語化しない限り思考の経緯は共有されない。

この設計では、完了通知にメモを添える形で思考の断片を渡している。

```
writer完了: shared/東アジア地政学/台湾海峡.md
メモ: 米中関係を中心軸に置いた。経済的相互依存の節は情報不足のため簡略化した。
```

これにより：
- **reviewer** は writer が「迷った箇所・情報不足」として挙げた部分を重点的に検証できる
- **editor** は writer・reviewer 双方の留保や残課題をサマリーの考察に反映できる

「成果物だけ渡す」から「成果物＋思考の断片を渡す」への転換は実装コストがほぼゼロである一方、エージェント間の情報の連鎖を大きく改善する。これはエージェント設計というより**コミュニケーション設計**の問題であり、人間のチームにも同様に当てはまる原則である。
